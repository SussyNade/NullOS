...
- The only processes that still always start at the root (`cwd_cluster = 0`) are ones with **no calling process to inherit from**: the very first shell, spawned directly by `kmain` at boot (`exec("shell", 0)` in `kernel/main.c`).
...
- Known, explicitly out-of-scope limitations (documented in code comments, not fixed here): `process_exit()` never frees a process's `cr3` or its mapped pages, forked children included; the page-copy assumes physical pages fall in the identity-mapped first 8MB, the same assumption `elf_load()` already makes
