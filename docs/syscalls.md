...
| 10 | `SYS_EXEC` | `exec(name, arg) → pid or -1` (`name` is resolved like any file open: the ramfs first, then FAT16 relative to the caller's current directory, so `dir/prog.elf` works; a FAT16 program is read from disk and can be at most 192 KB. `arg` is optional, retrieved by the new process via `SYS_GETARG`; `run <prog>` passes none, `edit <file>` passes the filename. Returns -1 if the program is not found or is not a valid ELF — see `docs/kernel.md`) |
