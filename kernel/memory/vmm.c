...
void vmm_init(void) {
    ...
    console_puts(msg(MSG_VMM_1_ZEROING_PD_AND));
    for (i = 0; i < 1024; i++) {
        pd[i]  = 0;
        pt0[i] = 0;
        pt1[i] = 0;
    ...
    console_puts(msg(MSG_VMM_2_MAPPING_0_4MB));
    for (i = 0; i < 1024; i++)
        pt0[i] = (i * PAGE_SIZE) | VMM_KERNEL | VMM_PRESENT;

    console_puts(msg(MSG_VMM_3_MAPPING_4MB_8MB));
    for (i = 0; i < 1024; i++)
    ...
    console_puts(msg(MSG_VMM_4_CR3_CR0_PG));
    ...
    paging_active = 1;
    console_puts(msg(MSG_VMM_5_OK));
}

void vmm_dump(void) {
    console_set_color(CONSOLE_CYAN, CONSOLE_BLACK);
    console_puts(msg(MSG_VMM_DUMP_TAG));
    console_set_color(CONSOLE_LIGHT_GREY, CONSOLE_BLACK);
    console_puts(msg(MSG_VMM_PTS_USED));
