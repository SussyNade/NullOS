...
- `crash <de|pf|gpf>` (Phase 20): a **debug tool** that makes the shell fault on purpose (`#DE` divide by zero, `#PF` read of `0xDEADBEEF`, `#GP` invalid segment selector) to test the crash handler and Safe Mode's crash screen; it takes the machine down (and restarts it into Safe Mode) — see `docs/safemode.md`.
