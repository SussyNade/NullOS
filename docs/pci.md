...
- `SYS_PCI_FIND (33)`: `pci_find(vendor, device) → 1/0` for userland (`nos_pci_find()`), added so `user/selftest.c` can check a specific device (the Intel 440FX host bridge, 8086:1237) instead of only "found ≥ 1". That selftest check is tied to QEMU's default `pc` machine and is expected to fail when Phase 24 moves QEMU to `-machine q35` — update the IDs then
